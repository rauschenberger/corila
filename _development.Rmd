...
functions <- c("calc_sign_prec","cv.corila",".assert",".combine_slopes",".deviance",".estim_initial_coefs",".expand_auxiliary",".folds",".is_adjacent",".mean_function",".residuals",".set_candidates",".simulate_outcome",".type")
notes <- list()
for(i in seq_along(functions)){
  notes[[i]] <- autotest::autotest_package(functions = functions[i],test=TRUE)
}

notes <- autotest::autotest_package(exclude = ".folds",test=TRUE)
...
data.frame(notes[notes$fn_name==".deviance",])

note <- autotest::autotest_package(functions=".simulate_outcome",test=TRUE)
