...
#'
#' #group <- seq_len(ncol(data$x_train))
#' #group <- LETTERS[data$group]
#'
...
#' y <- rnorm(n)
#' group <- rep(as.numeric(seq_len(q)), length.out = p)
#' primary <- rep(c(TRUE, FALSE), length.out = p)
#' nfolds <- 3
...
.validate <- function(na_action, x, y, group, primary, family, hyper,
  ...
  if (!is.null(foldid)) {
    ...
    if (!all(foldid %in% seq_len(max(foldid)))) {
      stop("All entries of vector 'foldid' must be in {1, ..., nfolds}.")
    }
    if (!all(seq_len(max(foldid)) %in% foldid)) {
      stop("All values in {1, ..., nfolds} must appear in 'foldid'.")
    }
  }
