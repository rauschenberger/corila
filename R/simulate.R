...
.simulate_lupi_data <- function(mode, n0 = 100, n1 = 1000, p = 200, q = 4,
                                plot = FALSE) {
  .assert(x = n0, type = "integer", min = 2)
  .assert(x = n1, type = "integer", min = 2)
  ...
  if (mode == "upstream") {
    ...
    for (j in seq_len(p / q)) {
      ...
      x[, sel.aux] <- x[, sel.pry] %*% t(sqrt(w)) + t(t(matrix(
        stats::rnorm(n * sum(sel.aux)),
        nrow = n,
        ncol = sum(sel.aux)
      )) * sqrt(1 - w))
    }
    ...
    for (j in seq_len(p / q)) {
      ...
      sel.aux <- group == j & !include
      w <- stats::runif(n = 1)
      #x[,sel.aux] <- sqrt(w)*stats::rnorm(n=n)+sqrt(1-w)*stats::rnorm(n=n*sum(sel.aux))
      #x[,sel.pry] <- rowSums(x[,sel.aux])
      x[, sel.aux] <- stats::rnorm(n = n * sum(sel.aux))
      w <- stats::runif(n = 4)
      #w <- c(1/3,1/3,1/3)
      #w <- stats::rgamma(n=4,shape=1,rate=1)
      w <- w / sum(w)
      #x[,sel.pry] <- sqrt(v)*(x[,sel.aux] %*% sqrt(w))+sqrt(1-v)*stats::rnorm(n=n) # originally with v=0.7
      #w <- c(1/4,1/4,1/4); x[,sel.pry] <- sqrt(1)*(x[,sel.aux] %*% sqrt(w))+sqrt(1/4)*stats::rnorm(n=n) # original # TRIAL (was above)
      #w <- stats::runif(n=4)
      #
      #w[4] <- 0
      #w <- c(0.1,0.2,0.4,0.3)
      #w <- w/sum(w)
      x[, sel.pry] <- cbind(x[, sel.aux], stats::rnorm(n = n)) %*% sqrt(w)
      #x[,sel.pry] <- x[,sel.aux] %*% sqrt(w)
    }
    beta <- (!include) * causal * abs(stats::rnorm(n = p))
    eta <- x %*% beta
    y <- eta + stats::rnorm(n = n, sd = 0.5 * stats::sd(eta))
    ...
    for (j in seq_len(p / q)) {
      sel.pry <- group == j & include
      sel.aux <- group == j & !include
      x[, sel.aux] <- stats::rnorm(n = n)
      # w <- c(0.2,0.5,0.8) # TRIAL # was c(0.7,0.5,0.3)#  stats::runif(n=q-1) # rep(x=0.9,times=q-1)
      w <- stats::runif(3)
      x[, sel.pry] <- x[, sel.aux] %*% t(sqrt(w)) + t(t(matrix(
        stats::rnorm(n * sum(sel.pry)),
        nrow = n,
        ncol = sum(sel.pry)
      )) * sqrt(1 - w))
    }
    ...
    for (j in seq(from = 2, to = q)) {
      list[[j]] <- sqrt(w[j]) * list[[j - 1]] + sqrt(1 - w[j]) * stats::rnorm(n = n * p / q)
    }
  ...
  graphics::image(t(cor(x)[p:1, ]))
  warning("Function should return y_train, y_test, x_train, and x_test!")
